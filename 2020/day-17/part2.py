...
Vector4D = namedtuple("Vector4D", ["x", "y", "z", "w"])


def add_vectors(a: Vector4D, b: Vector4D) -> Vector4D:
...
def load_cells_from_input(lines):
    for y, line in enumerate(lines):
        for x, c in enumerate(line):
            if c == "#":
                yield Vector4D(x=x, y=y, z=0, w=0)
...
def print_grid(grid):
    ...
    for z in z_range:
        ...
        for y in y_range:
            print(
                "".join(
                    "#" if Vector4D(x=x, y=y, z=z) in grid else "." for x in x_range
                )
            )
